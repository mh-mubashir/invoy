...
export default function App() {
  ...
  async function sendInvoiceEmail(invoiceData: any, recipientEmail: string) {
    try {
      const res = await fetch('/ai-invoice/send-email', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (data.status === 'ok') {
        setMsgs(m => [...m, <Message role="ai" key={Date.now()}>
          ✅ Email sent successfully to <strong>{recipientEmail}</strong>
        </Message>])
      } else {
        setMsgs(m => [...m, <Message role="ai" key={Date.now()}>
      ...
      setMsgs(m => [...m, <Message role="ai" key={Date.now()}>Failed to send email.</Message>])
    }
  }
  ...
  async function finalizeInvoice(allocData: any) {
    try {
      ...
      const data = await res.json()
      const clientEmail = allocData.client_name.includes('@') ? allocData.client_name : ''
      setMsgs(m => [...m, <Message role="ai" key={Date.now()}>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button onClick={()=>setPreviewUrl(data.path)} className="rounded-xl px-4 py-3 bg-gradient-to-r from-sky-500 to-blue-600 hover:from-sky-600 hover:to-blue-700 text-white font-semibold shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
              View
            </button>
            {clientEmail && (
              <button onClick={()=>sendInvoiceEmail(data, clientEmail)} className="rounded-xl px-4 py-3 bg-gradient-to-r from-violet-500 to-purple-600 hover:from-violet-600 hover:to-purple-700 text-white font-semibold shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2">
        ...
        {/* Sidebar */}
          ...
          <aside className="w-[540px] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-lg flex-shrink-0 overflow-hidden flex flex-col">
            <div className="px-6 py-5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
              ...
              {previewUrl && (
                <a href={previewUrl.replace('.html', '.pdf')} download className="flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-900 dark:bg-slate-800 hover:bg-slate-800 dark:hover:bg-slate-700 text-white text-sm font-medium shadow-md hover:shadow-lg transition-all">
